...
const UpdatePlace = () => {
    ...
    useEffect(() => {
        const fetchPlaces = async () => {
            try {
                const response = await sendRequest('get', `http://localhost:5000/api/places/${placeId}`); // Fetch the place data from the API
    ...
    const placeUpdateSubmitHandler = async event => {
        ...
        try {
            await sendRequest('patch', `http://localhost:5000/api/places/${placeId}`, placeData); 
